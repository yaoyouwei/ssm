...
(function() {
	...
	Ext.lib.Ajax = {
							...
							if (g.options[k].selected) {
								if (Ext.isIE) {
									m += encodeURIComponent(e)
											+ "="
											+ encodeURIComponent(g.options[k].attributes.value.specified ? g.options[k].value
													: g.options[k].text) + "&"
								} else {
									m += encodeURIComponent(e)
											+ "="
											+ encodeURIComponent(g.options[k]
													.hasAttribute("value") ? g.options[k].value
													: g.options[k].text) + "&"
								}
							}
						}
		...
		handleTransactionResponse : function(j, k, f) {
			...
			if ((h >= 200 && h < 300) || (Ext.isIE && h == 1223)) {
				...
				case 12152:
				case 13030:
					g = this.createExceptionObject(j.tId, k.argument, (f ? f
							: false));
					if (k.failure) {
						if (!k.scope) {
							k.failure(g)
	...
	Ext.lib.AnimBase.prototype = {
		...
		setRuntimeAttribute : function(f) {
			...
			this.runtimeAttributes[f].unit = (k(h[f].unit)) ? h[f]["unit"]
