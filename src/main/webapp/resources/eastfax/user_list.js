...
Ext.onReady(function() {
			...
			new Ext.app.SearchField({
		        ...
		        listeners:{
			    	'keyup' :{
			            fn: function (f,e){
			            	console.log(f);
			            	var o = {name:'hello',age:13};
			            	console.log(o);
			            	console.log(e);
			                console.log(f.paramName);
			            	f.onTrigger2Click();//触发回车事件
				    	},
			            scope: this,
			            delay: 100
			        }
	...
	function onBtnSyncData(){
		...
		Ext.Ajax.request({
			url : CTX+'user/mgnt/list.do?c_syncUserList',
			method : 'POST',
			success : function(response, parameter) {
				...
				console.log(result);
				if (result.success) {
					Ext.MessageBox.show({ title : '成功', msg : "同步成功!" });
					store.load({
						params : {
							start : 0,
							limit : pageSize
